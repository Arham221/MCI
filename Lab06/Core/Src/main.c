...
//     {
//         HAL_GPIO_TogglePin(GPIOE, LD3_Pin);  // Toggle LED for visual feedback
//         if (sample_index >= SAMPLE_SIZE) return;   // already have enough samples, ignore

//         if (edge_state == 0)

//         {
//             // First edge: reset and start the timer
...
//         else
//         {
//             // Second edge: stop timer, store elapsed count
//             HAL_TIM_Base_Stop(&htim2);
//             capture_values[sample_index] = __HAL_TIM_GET_COUNTER(&htim2);
//             sample_index++;
//             edge_state = 0;   // reset for next pair
...
int main(void)
{
  ...
  //       float avg_period = (float)sum / SAMPLE_SIZE;   // in timer ticks
  //       // TIM2 clock: 8 MHz APB / (prescaler+1) = 8MHz/10 = 800kHz tick rate
  //       // manual's formula: f = 8e6 / (2 * average_period)
  //       float frequency = 48000000.0f / avg_period;
