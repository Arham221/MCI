...
//   //Read current counter ot Tim2 until ms reached
//   while (__HAL_TIM_GET_COUNTER(&htim2) < ms){}

//   //Stop TIM2 from counting
...
int main(void)
{
  ...
  // // Configure PE8, PE9, and PE10 as push-pull outputs
  // GPIO_InitTypeDef GPIO_InitStruct = {0};
  ...
  // HAL_TIM_Base_Start_IT(&htim2);
    ...
    //task 4 begin
    HAL_UART_Transmit(&huart2, (uint8_t*)"System booted\r\n", 16, HAL_MAX_DELAY);
    //task4 end
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    //   //formatting output into character
    //   char buffer[50];
...
//task3
// void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
//     // Verify the interrupt was triggered by TIM2[cite: 1]
//     if (htim->Instance == TIM2) {
        
//         // Increment all counters (since this function runs every 1 ms)
//         countA++;
//         countB++;
//         countC++;
...
//         // countC -> 100 ms (5 Hz)
//         if (countC >= 300) {
//             HAL_GPIO_TogglePin(GPIOE, GPIO_PIN_10); // Toggle Orange LED
//             countC = 0;                             // Reset counter
//         }
//     }
// }
//task3 end
